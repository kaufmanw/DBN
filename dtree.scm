...
;;   Each entry in examples is a pair whose car is a label (any Scheme value)  
;;      whose and cdr is an association list. 
;;   Each association list in examples has identical keys (and in the same
;;       order), but potentially different values.
;;   Each key in attrib-values is a key in the example association list
;;   Each value of an attribute in examples a member of the corresponding 
;;      association list values for attrib-values
;;   attrib is a key in attrib-values (an by extension, in the association 
;;      list for an example)
;;
;; Postconditions
;;   gain >= 0
